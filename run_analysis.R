## check to see if dataset is in working dir. if not download it and unzip it.
if(!file.exists("./UCI HAR Dataset")){
  url = "https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip" 
  download.file(url,destfile="./courseProject.zip",method="curl") 
...
# calculate the means
means.list <- lapply(s,function(x) colMeans(x[,newcolnames[1:73]]))

#convert to dataframe
tidy.table.dt  <- do.call(cbind.data.frame, means.list)

# save the dataframe to disk
write.csv(tidy.table.dt,file="./courseProjectTidyTable.csv")
